...
class TestOffice(TestCase):
    ...
    def test_initial(self):
        root = Office(name="root", node_id=0)
        root.save()

        office_a = root.add_child("A")  # A=1
        office_b = root.add_child("B")  # B=1, A=2

        # all_offices = list(Office.objects.all().order_by("node_id"))
        self.assertEqual(2, Office.objects.get(pk=office_a.pk).node_id)
        ...
        self.assertEqual(7, Office.objects.get(pk=office_aa.pk).node_id)
